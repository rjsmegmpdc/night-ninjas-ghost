...
> **CURRENT STATE — M3 + Kiero (2026-07). Sections below are partially
> stale.** The shipped system is Material 3 dynamic colour, not the token
> tables in this doc: tokens are generated by
> `scripts/generate-m3-tokens.mjs` into `src/m3-tokens.css` and mapped to
> Tailwind v4 utilities in `src/index.css` (`tailwind.config.ts` is dead —
> wrong content globs, undefined `--nn-*` vars). As of the **Kiero
> rebrand** the M3 seed and `--m3-brand` are **Kiero teal `#2DD9CE`**
> (was GHOST orange `#FF5F00`); Storm keeps its blue seed; light (dawn)
> brand deepens to `#0D9488`. Card radius is 20px (`m3-card`,
> `--radius-2xl`). Style reference: the Kiero screenshots in the v2 repo's
> `brief/screenshots/AI/`. Where this doc says orange-accent or 12px
> cards, the code (and this note) wins.
...
1. The brand seed is Kiero teal `#2DD9CE` (M3-generated roles; dawn brand
   deepens to `#0D9488`; Storm's blue seed is a deliberate exception).
2. Navigation is the M3 layout: mobile bottom nav + top app bar, desktop
   rail (see `src/components/nav/TopNav.tsx`).
3. Cards are 20px radius (`m3-card` / `rounded-2xl`). Pills and primary
   CTAs are `rounded-full`.
