 ...
 *
 * Each block is a pure function `(template, context, config, zones) => template`.
...
export function applyCommitmentScaling(
  ...
  if (impact === 'no-training') {
    return {
      ...
      days: template.days.map((d) => ({
        dow: d.dow,
        sessions: [{ label: 'Rest (commitment)', type: 'rest' as const }],
      })),
      adaptations: [
        ...(template.adaptations ?? []),
        { kind: 'no-training', label: 'No training week', detail: describeEvents(overlapping) },
      ],
    };
  }
  ...
  if (impact === 'travel-only') {
    return {
      ...
      adaptations: [
        ...(template.adaptations ?? []),
        { kind: 'travel-only', label: 'Travel-only week', detail: describeEvents(overlapping) },
      ],
    };
  }

  if (impact === 'reduced') {
    return scaleVolume(template, config.reducedFactor, {
      kind: 'reduced',
...
export function substituteRecurringSessions(
  ...
  const days = template.days.map((day) => {
    ...
    return { ...day, sessions: [recurringToSession(recurring, zones)] };
  });
  ...
  return {
    ...
    adaptations: [
      ...(template.adaptations ?? []),
      {
        ...
        detail: context.recurringSessions.map((r) => `${dowName(r.dow)} ${r.label}`).join(' · '),
      },
    ],
  };
}
...
function recurringToSession(
  ...
  switch (r.intent) {
    case 'tempo':
      return { label: r.label, type: 'tempo', paceZone: zones.threshold, distanceKmMin: r.distanceKm, distanceKmMax: r.distanceKm };
    case 'interval':
      return { label: r.label, type: 'interval', paceZone: zones.interval, distanceKmMin: r.distanceKm, distanceKmMax: r.distanceKm };
    case 'long':
      return { label: r.label, type: 'long', paceZone: zones.long, distanceKmMin: r.distanceKm, distanceKmMax: r.distanceKm };
    default:
      return {
        label: r.label, type: 'easy', paceZone: zones.easy,
        distanceKmMin: r.distanceKm, distanceKmMax: r.distanceKm,
        durationMinMin: r.durationMin, durationMinMax: r.durationMin,
      };
  }
}
...
export function applyTuneupAdjustments(
  ...
  if (!config.enabled || !context || context.tuneupRaces.length === 0) return template;
  ...
  for (const race of context.tuneupRaces) {
    ...
    days = days.map((d) => {
      if (d.dow === raceDow) {
        return {
          dow: d.dow,
          sessions: [{ label: `Tune-up race · ${race.name}`, type: 'long' as const, distanceKmMin: race.distanceKm, distanceKmMax: race.distanceKm }],
        };
      }
      const distanceFromRace = Math.abs(d.dow - raceDow);
      if (d.dow < raceDow && distanceFromRace <= config.taperDays) {
        return { dow: d.dow, sessions: [{ label: 'Pre-race easy', type: 'easy' as const, distanceKmMin: 3, distanceKmMax: 6 }] };
      }
      if (d.dow > raceDow && distanceFromRace <= config.recoveryDays) {
        return { dow: d.dow, sessions: [{ label: 'Recovery', type: 'recovery' as const, distanceKmMin: 0, distanceKmMax: 5 }] };
      }
      return d;
    });
  }
  ...
  return {
    ...
    adaptations: [
      ...(template.adaptations ?? []),
      { kind: 'tuneup-race', label: 'Tune-up race', detail: context.tuneupRaces.map((r) => `${r.name} ${r.date}`).join('; ') },
    ],
  };
}
...
export function annotateNinjaLoopDays(
  ...
  return {
    ...
    adaptations: [
      ...(template.adaptations ?? []),
      { kind: 'ninja-loop', label: 'Ninja Loop', detail: `${context.ninjaLoopDays.map(dowName).join(', ')} — group meet day` },
    ],
  };
}
...
function scaleVolume(template: WeekTemplate, factor: number, adaptation: WeekAdaptation): WeekTemplate {
  return {
    ...template,
...
function describeEvents(events: WeekEvent[]): string {
  return events.map((e) => `${e.type.replace('-', ' ')} (${e.startDate} → ${e.endDate})`).join('; ');
}
...
export function applyStructuredCalendar(
  ...
  if (config.honourRecurringSessions) t = substituteRecurringSessions(t, context, zones);
  t = applyTuneupAdjustments(t, context, config.tuneups);
  if (config.annotateNinjaLoops) t = annotateNinjaLoopDays(t, context);
  return t;
}
